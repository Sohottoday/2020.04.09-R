...
고수준 함수 - plot(), boxplot(), hist(), pie(), barplot()
         => 하나의 화면에 하나만 사용 가능하다.
		 2개 이상을 쓰려면 레이아웃을 나눠야 함.
		 
저수준 함수 - 고수준 함수를 꾸며주는 역할

par => 화면 설정하는 함수.
par(mar=c(1,1,1,1), mfrow=c(4,2))
mar => 상하좌우 마진 설정 ex)mar=c(1,1,1,1)
mfrow => 한 화면에서의 레이아웃 나누는 함수. ex) mfrow=c(4,2) 4줄 2개로 8개 레이아웃

레이아웃을 나눈 다음 원상태로 복귀시키려면 par 함수를 통해 mfrow로 다시 1행 1열로 맞춰야 한다.

axis를 통해 x축 y축을 설정할 수 있다.
axix(1 은 x축 2 는 y축)
axis(1, at=1:10, lab=c("01","02","03","04", "05","06","07","08","09","10")) # x축추가
axis(2, at=c(0,2,4,6,8,10)) # y축추가
lab = 라벨 - 직접 라벨도 설정할 수 있다.

legend = 범례를 그리는 함수.

prob = 

boxplot 에서의 at => 출력할 때 구분해서 출력함.
라벨 가로 세로.
...
# clockwise = T를 주면 시계방향으로 진행하며 12시부터 시작,
# 기본값(디폴트값)은 3시부터 반시계 방향으로 진행된다.
